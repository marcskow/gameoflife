package pl.edu.marcskow.gameoflife.gui.controller;
...
import pl.edu.marcskow.gameoflife.gui.helpers.DisplayService;
import pl.edu.marcskow.gameoflife.gui.helpers.GameSettings;
import pl.edu.marcskow.gameoflife.gui.helpers.GridModel;
import pl.edu.marcskow.gameoflife.gui.helpers.OnStartService;
import pl.edu.marcskow.gameoflife.gui.structures.Structures;
...
public class AutomatonController extends AnchorPane implements Initializable {
    ...
    private final ObservableList<String> gameOfLifeStructureList = FXCollections.observableArrayList("Next", "Glider", "Dakota", "Gosper Glider Gun");
    private final ObservableList<String> langtonStructureList = FXCollections.observableArrayList("Next"); // TODO: 2016-01-02
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        gridModel = new GridModel(GameSettings.COLUMNS, GameSettings.ROWS, GameSettings.AUTOMATON);
    ...
    public void structureBoxOnChangeListener(){
        GameSettings.STRUCTURE = structureChoiceBox.getSelectionModel().getSelectedItem();
    }

    @FXML
    public void neighborhoodBoxOnChangeListener(){
        GameSettings.NEIGHBORHOOD = neighbourhoodChoiceBox.getSelectionModel().getSelectedItem();
    }
    ...
    public void sizeBoxOnChangeListener(){
        ...
        switch (newSize) {
            case "10x8": GameSettings.setDimension(10, 8); break;
            case "20x16": GameSettings.setDimension(20, 16); break;
            case "40x32": GameSettings.setDimension(40, 32); break;
            case "50x40": GameSettings.setDimension(50, 40); break;
            case "100x80": GameSettings.setDimension(100, 80); break;
            case "125x100": GameSettings.setDimension(125, 100); break;
        }
        gridModel.setDimension(GameSettings.COLUMNS,GameSettings.ROWS);
        gridModel.prepareGridAfterAutomatonChange();
        updateGridDisplay();
    }
    ...
    public void wrappingOnChangeListener(){
        if(wrappingCheckBox.isSelected()){
            GameSettings.ISWRAPPING = true;
        }else{
            GameSettings.ISWRAPPING = false;
        }
    }
    ...
    public void mouseClickedOnGrid(MouseEvent event) {
        ...
        if(action.equals("Next")){
            ...
            drowRectangle(DisplayService.setColorToAutomatonType(GameSettings.AUTOMATON,gridModel.getCellFromMap(roundedX,roundedY)),roundedX,roundedY);
        }
            ...
            gridModel.putElementOnMap(roundedX,roundedY,DisplayService.setDefaultStateToAutomatonType(GameSettings.AUTOMATON));
        }
        else if(GameSettings.AUTOMATON.equals("Game Of Life")) {
            gridModel.addStructure(structures.returnStructure(action,GameSettings.AUTOMATON,BinaryState.DEAD,roundedX,roundedY));
            updateGridDisplay();
        }
        else if(GameSettings.AUTOMATON.equals("QuadLife")){
            gridModel.addStructure(structures.returnStructure(action, GameSettings.AUTOMATON,QuadState.DEAD,roundedX,roundedY));
            updateGridDisplay();
        }
    }
    ...
    private void drowRectangle(Color color, int x, int y) {
        ...
        rectangle.setStroke(GameSettings.DEFAULT_STROKE_COLOR);
        rectangle.setStrokeWidth(GameSettings.DEFAULT_STROKE_WIDTH);
    ...
    private void changeAutomatonType(String newType, ObservableList<String> structure, ObservableList<String> neighbourhood, boolean areGameOfLifeRulesVisible, boolean areElementaryRulesVisible) {
        GameSettings.AUTOMATON = newType;
